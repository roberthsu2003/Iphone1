...
class DataSource{
    ...
    func selectedContinent(continentName:String) -> [City]{
        createDb();
        let selectedSqlString = "select * from city where continent = ?";
        var statement:OpaquePointer!;
        if sqlite3_prepare_v2(DataSource.db, selectedSqlString, -1, &statement, nil) == SQLITE_OK{
            print("prepare_v2 ok");
        }else{
        ...
        sqlite3_bind_text(statement, 1, (continentName as NSString).utf8String, -1, nil);
