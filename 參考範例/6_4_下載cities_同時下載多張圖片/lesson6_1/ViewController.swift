...
class ViewController: UITableViewController {
    ...
    var cityInfos = [Root.City]() //先建立一個空的
    var indicatorView = {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "goImageView"{
            ...
            imageViewController.infos = cityInfos
...
extension ViewController:DataSourceDelegate{
    ...
    func finishDownLoad(data:[Root.City]){
        ...
        if self.navigationItem.rightBarButtonItem == nil{
            self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "imageCity", primaryAction: UIAction.init(handler: { _ in
                self.performSegue(withIdentifier: "goImageView", sender: nil)
            }))
        }
        print("完成下載")
    }
...
extension ViewController{
    ...
    ) -> UITableViewCell
    {
        ...
        let city = cityInfos[indexPath.row]
        
        configure.text = city.cityName
        configure.secondaryText = city.country
